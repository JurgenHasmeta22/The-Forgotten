...
func _physics_process(_delta):
	...
	# Process stamina consumption for sprinting
	if sprinting:
		_physics_process_sprint(_delta)

	# Process stamina consumption for blocking
	if guarding and stamina_system:
		if not stamina_system.use_stamina(stamina_system.block_cost * _delta):
			# Not enough stamina to continue blocking
			end_guard()
...
func attack():
	if stamina_system and not stamina_system.use_stamina(stamina_system.attack_cost):
...
func attack_strong():
	...
	if stamina_system and not stamina_system.use_stamina(stamina_system.attack_cost * 1.5):
		# Not enough stamina for strong attack
		stamina_depleted.emit()
		return
...
func dodge_or_sprint():
	if sprint_timer.is_stopped():
		...
		if !dodging && input_dir:
			if stamina_system and stamina_system.has_enough_stamina(stamina_system.sprint_cost):
				sprinting = true
...
func _physics_process_sprint(delta):
	# Called from _physics_process to consume stamina while sprinting
	if sprinting and stamina_system:
		if not stamina_system.use_stamina(stamina_system.sprint_cost * delta):
			# Not enough stamina to continue sprinting
			end_sprint()
			stamina_depleted.emit()
...
func dodge():
	...
	if stamina_system and not stamina_system.use_stamina(stamina_system.dodge_cost):
...
func start_guard(): # Guarding, and for a short window, parring is possible
	if stamina_system and not stamina_system.has_enough_stamina(stamina_system.block_cost):
		# Not enough stamina to start guarding
		stamina_depleted.emit()
		return
