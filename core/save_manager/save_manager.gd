...
func write_save(content, slot: int = 0):
	...
	print("Game saved to slot " + str(save_slot))

func read_save(slot: int = 0):
	...
	if not FileAccess.file_exists(save_path):
		...
		return create_new_save_file(save_slot)
...
# region "Load game and ready func
func _ready():
...
func load_game(slot: int = 0) -> bool:
	...
	# Load the save data
	var save_data = read_save(save_slot)

	if save_data.is_empty():
		push_error("Failed to load save data from slot " + str(save_slot))
		return false
